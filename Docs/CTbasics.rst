...
that plugs into the CarbixEnergyMonitor.
...
CTs that are suited to use with the CarbixEnergyMonitor in typical
scenarios.
...
create a safety hazard. When plugged into the CarbixEnergyMonitor, the
...
CarbixEnergyMonitor will accept many different CTs from different manufacturers.
...
And so it is for the Echun and AccuEnergy CTs that CarbixEnergyMonitor, Inc makes available.
...
typical CarbixEnergyMonitor installations use a single reference
...
(or 230V European type) circuits, CarbixEnergyMonitor will automatically correct
...
directing CarbixEnergyMonitor to double the voltage value to report
...
The CTs can be connected to two individual CarbixEnergyMonitor inputs
...
splitter and fed into a single CarbixEnergyMonitor input. When combining
