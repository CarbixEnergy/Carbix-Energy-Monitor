...
The CarbixEnergyMonitor has a bi-color external LED indicator that is usefull in
...
the CarbixEnergyMonitor can run indefinitely without WiFi.
If you are logging to Emoncms or another server, 
that will be suspended, but CarbixEnergyMonitor will recognize
...
To determine if CarbixEnergyMonitor is connected to the WiFi network,
try to run the CarbixEnergyMonitor app from a browser on a device that
...
To be sure, restart the CarbixEnergyMonitor by disconnecting the 5VDC power momentarily
...
Updates are published from time to time and most CarbixEnergyMonitor subscribe
...
While the download is in progress, the CarbixEnergyMonitor does nothing else
and will blink the led RED-GREEN continuously. 
At the completion of the download, 
the release will be installed and your CarbixEnergyMonitor will restart.
...
CarbixEnergyMonitor is having trouble connecting to the WiFi network.
If this is a new CarbixEnergyMonitor, or the network has changed,
...
is not running. CarbixEnergyMonitor can run without WiFi,
...
CarbixEnergyMonitor is having trouble accessing the SDcard inside the device.
...
the CarbixEnergyMonitor SD card socket and the device will need to be replaced.
