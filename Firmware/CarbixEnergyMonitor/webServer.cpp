...
void handleFileUpload(){
  ...
  if(upload.status == UPLOAD_FILE_START){
    ...
    if(upload.filename.equals(F(CARBIX_CONFIG_PATH)) ||
       upload.filename.startsWith(F(CARBIX_SYSTEM_DIR))){
      returnFail("Protected", 403);
      return;
    }
    if( ! authenticate(authAdmin)) return;
    if(upload.filename.equals(F(CARBIX_CONFIG_NEW_PATH))){
...
void handleDelete(){
  ...
  if(path == F(CARBIX_CONFIG_PATH) ||
     path == (F(CARBIX_CURRENT_LOG_PATH)) ||
     path == (F(CARBIX_HISTORY_LOG_PATH)) ||
     path == (F(CARBIX_AUTH_PATH)))
  {
...
/************************************************************************************************
 * 
 * Following handlers added to WebServer for CarbixEnergyMonitor specific requests
 * 
 **********************************************************************************************/
...
void handleStatus(){
  ...
  String response = "";
  {
    ...
    if(server.hasArg(F("stats"))){
      ...
      trace(T_WEB,14);
      stats.set(F("version"),CARBIX_ENERGY_MONITOR_VERSION);
      trace(T_WEB,14);
  ...
  if(server.hasArg(F("outputs"))){
    ...
    if(server.hasArg(F("datalogs"))){
      ...
      while(script){
        CarbixLog *log = ((integrator *)script->getParm())->get_log();
...
void handleCommand(){
  ...
  if(server.hasArg(F("deletelog"))) {
    ...
    if(arg == "current"){
      trace(T_WEB,21); 
      Current_log.end();
      deleteRecursive(CARBIX_CURRENT_LOG_PATH);
    } 
    else if(arg == "history"){
      trace(T_WEB,22); 
      History_log.end();
      deleteRecursive(CARBIX_HISTORY_LOG_PATH);
    }
    else if(arg == "both"){
      trace(T_WEB,23);
      Current_log.end();
