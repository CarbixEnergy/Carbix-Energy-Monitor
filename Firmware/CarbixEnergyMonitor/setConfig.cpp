...
bool updateConfig(const char *newConfig){
  if(setConfig(newConfig)){
    if(SD.exists(F(CARBIX_CONFIG_OLD_PATH))){
      SD.remove(F(CARBIX_CONFIG_OLD_PATH));
    }
    SDFS.rename(F(CARBIX_CONFIG_PATH), F(CARBIX_CONFIG_OLD_PATH));
    SDFS.rename(newConfig,F(CARBIX_CONFIG_PATH));
    return true;
  }
  setConfig(CARBIX_CONFIG_PATH);
  return false;
}
...
//*************************************************************************************************
bool recoverConfig(){
  if(SD.exists(F(CARBIX_CONFIG_NEW_PATH)) && setConfig(CARBIX_CONFIG_NEW_PATH)){
    return updateConfig(CARBIX_CONFIG_NEW_PATH);
    log("using new config+1.");
  }
  if(SD.exists(F(CARBIX_CONFIG_OLD_PATH)) && setConfig(CARBIX_CONFIG_OLD_PATH)){
    SD.remove(CARBIX_CONFIG_PATH);
    SDFS.rename(F(CARBIX_CONFIG_OLD_PATH), F(CARBIX_CONFIG_PATH));
...
bool configDevice(const char* JsonStr){
  ...
  if(maxInputs == 0) {
    inputChannel = new CarbixInputChannel*[channels];
    for(int i=0; i<channels; i++){
      inputChannel[i] = new CarbixInputChannel(i);
  ...
  if(device.containsKey(F("chanaref"))){
    for(int i=0; i<MIN(maxInputs,device[F("chanaddr")].size()); i++){
      inputChannel[i]->_aRef = device[F("chanaref")][i].as<unsigned int>();
          ...
          // else, the default in the CarbixInputChannel constructor prevails.
 ...
 * 5) Set pointers in each carbixInputChannel to their corresponding dynamic phase arrays
 * 
 * **********************************************************************************************/
...
bool configMasterPhaseArray(){
  ...
  trace(T_CONFIG, 20, 2);
  File tableFile;
  String tableFileURL = CARBIX_TABLE_PATH;
  ...
  trace(T_CONFIG, 20, 6);
  for(int i=0; i<maxInputs; i++){
    CarbixInputChannel* input = inputChannel[i];
    input->_p50 = nullptr;
    input->_p60 = nullptr;
    if(input->isActive()){
      for(int t=0; t<models; t++){
