...
// EEPROM.begin(EEsize);
// memcpy(EE->id, "CarbixEnergyMonitor", 8);
// EE->EEversion = 0;
...
if( ! memcmp(EE->id, "CarbixEnergyMonitor", 8)){
  if(EE->EEversion > 0){
...
log("CarbixEnergyMonitor %d.%s, Firmware version %s", deviceMajorVersion, deviceMajorVersion < 5 ? "x" : String(deviceMinorVersion).c_str(), CARBIX_ENERGY_MONITOR_VERSION);

//**************************************** Install any pending updates ******************************

copyUpdate(String(CARBIX_ENERGY_MONITOR_VERSION));
  ...
  if( (! RTCrunning) || powerFailRestart){
    ...
    while(WiFi.status() != WL_CONNECTED){
      if(millis() > autoConnectTimeout){
        ...
        String ssid = "carbix" + String(ESP.getChipId());
