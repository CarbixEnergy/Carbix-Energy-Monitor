  ...
  switch(state){
    ...
    case waitVersion: {
      ...
      if(classes.containsKey(updateClass)){
        ...
        if(updateVersion.equals(CARBIX_ENERGY_MONITOR_VERSION)){
          if( ! upToDate){
          ...
          log("Updater: Update from %s to %s", CARBIX_ENERGY_MONITOR_VERSION, updateVersion.c_str());
          state = createFile;
          return 1;
        }
      }
    ...
    case download: {
      ...
      String URL = String(FPSTR(updateURL_P)) + String(F(CARBIX_VERSIONS_DIR)) + updateVersion + ".bin";
      request->setDebug(false);
      trace(T_UPDATE,6);   
      if( ! request->open("GET", URL.c_str())){
    ...
    case waitTable: {
      ...
      if(responseCode == 200){
        if(SD.exists(F(CARBIX_NEW_TABLE_PATH))) SD.remove(F(CARBIX_NEW_TABLE_PATH));
        File tableFile = SD.open(F(CARBIX_NEW_TABLE_PATH), FILE_WRITE);
        if(!tableFile){
          return false;
        }
        uint8_t buf[512];
        ...
        tableFile.close();
        SD.remove(F(CARBIX_TABLE_PATH));
        SDFS.rename(F(CARBIX_NEW_TABLE_PATH), F(CARBIX_TABLE_PATH));
        tableVersion = getTablesVersion();
      }
      delete request;
      request = nullptr;
 ...
 * 
 * After unpacking, the signature on the file is verified using the CarbixEnergyMonitor public key.
 * Only release files from CarbixEnergyMonitor.com can be verified because the private-key is needed to
...
bool unpackUpdate(String version){
  ...
  while(binarySize){
    ...
    while(fileSize){
      ...
      if(carbixenergymonitorBin) md5.add(buff, MIN(fileSize, buffSize));
 ...
 * Directory contains the ESP8266 binary "carbixenergymonitor.bin" with an md5 hash appended.
...
long getTablesVersion(){
  File tableFile;
  tableFile = SD.open(F(CARBIX_TABLE_PATH), FILE_READ);
  if(!tableFile){
    return -1;
  }
  DynamicJsonBuffer Json;
