...
void samplePower(int channel, int overSample){
  ...
  trace(T_POWER,1);
  CarbixInputChannel* Ichannel = inputChannel[channel];
  CarbixInputChannel* Vchannel = inputChannel[Ichannel->_vchannel];
...
float sampleVoltage(uint8_t Vchan, float Vcal){
  CarbixInputChannel* Vchannel = inputChannel[Vchan];
  uint32_t sumVsq = 0;
  int retries = 0;
...
//   CarbixInputChannel* Vchannel = inputChannel[Vchan];
//   CarbixInputChannel* Ichannel = inputChannel[Ichan];
...
float samplePhase(uint8_t Ichan, uint8_t Cchan, int shift, double *VPri, double *VSec){
  ...
  CarbixInputChannel* Ichannel = inputChannel[Ichan];
  CarbixInputChannel* Cchannel = inputChannel[Cchan];
