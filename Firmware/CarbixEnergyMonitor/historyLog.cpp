 ...
 * 16 years.  Moreover, because it is built by this process using the carbixlog, it has
...
uint32_t historyLog(struct serviceBlock* _serviceBlock){
  ...
  static CarbixLogRecord* logRecord = nullptr;
  trace(T_history,0);  
  ...
  switch(state){
    ...
    case initialize: {
      trace(T_history,1);
        ...
        // If carbixLog not open or empty, check back later.
      ...
      trace(T_history,2);   
      if(int rtc = History_log.begin(CARBIX_HISTORY_LOG_PATH)){
      ...
      if(History_log.firstKey() != 0){    
        ...
        // New history log, start with first even increment in CarbixLog

      else {
        if( ! logRecord) logRecord = new CarbixLogRecord;
