 ...
 * The CarbixLog class handles the SD file work.
 ...
 * VT channel bucket.
 * 
 * Entries are only made in real time when the CarbixEnergyMonitor is running, so they are not periodic,
 * but they are ordered.  It is relatively quick to find any record by key (UNIXtime) and a 
 * readKEY method is provided in the CarbixLog class.
 * 
 ...
 uint32_t dataLog(struct serviceBlock* _serviceBlock){
  ...
  switch(state){
    ...
    case initialize: {
      ...
      // Initialize the CarbixLog class
      
      if(int rtc = Current_log.begin(CARBIX_CURRENT_LOG_PATH)){
        log("dataLog: Log file open failed. %d", rtc);
        dropDead();
      }

      // Initialize the CarbixLogRecord accums in case no context.
      ...
      if(Current_log.fileSize() == 0){
        ...
        if(History_log.begin(CARBIX_HISTORY_LOG_PATH) == 0 && History_log.fileSize() > 0){
    ...
    case synchronize: {
      ...
      msThen = millis();
      for(int i=0; i<maxInputs; i++){
        CarbixInputChannel* _input = inputChannel[i];
        if(_input){
    ...
    case logData: {
      ...
      CarbixLogRecord *oldRecord = nullptr;
      if(integrations->count()){
        oldRecord = new CarbixLogRecord;
        memcpy(oldRecord, logRecord, sizeof(CarbixLogRecord));
      ...
      // Logging data is the primary purpose of CarbixEnergyMonitor.
...
/******************************************************************************
 * logReadKey(carbixLogRecord) - read a keyed record from the combined log
 * 
