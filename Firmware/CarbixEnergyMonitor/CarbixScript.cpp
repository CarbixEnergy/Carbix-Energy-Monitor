...
double  Script::run(CarbixLogRecord* oldRec, CarbixLogRecord* newRec, const char* overideUnits){
        for(int i=0; i<unitsNone; i++){
...
double  Script::run(CarbixLogRecord* oldRec, CarbixLogRecord* newRec){
        return run(oldRec, newRec, _units);
}

double  Script::run(CarbixLogRecord* oldRec, CarbixLogRecord* newRec, units Units){
...
double  Script::runRecursive(uint8_t** tokens, CarbixLogRecord* oldRec, CarbixLogRecord* newRec, units Units){
  double elapsedHours = 1.0;
  if(oldRec){
    elapsedHours = newRec->logHours - oldRec->logHours;
  }
