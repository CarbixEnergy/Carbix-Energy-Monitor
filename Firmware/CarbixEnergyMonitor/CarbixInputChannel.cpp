...
void CarbixInputChannel::ageBuckets(uint32_t timeNow) {
...
void CarbixInputChannel::setVoltage(float volts, float Hz){
    if(_type != channelTypeVoltage) return;
    dataBucket.Hz = Hz;	
    setVoltage(volts);
}

void CarbixInputChannel::setVoltage(float volts){
    if(_type != channelTypeVoltage) return;
    dataBucket.volts = volts;
    ageBuckets(millis());
}

void CarbixInputChannel::setHz(float Hz){
    if(_type != channelTypeVoltage) return;
    dataBucket.Hz = Hz;
}

void CarbixInputChannel::setPower(float watts, float VA){
...
float CarbixInputChannel::getPhase(const float var){
    float frequency;
    float phase = _phase;                                   
...
float CarbixInputChannel::lookupPhase(int16_t* pArray, const float var){
