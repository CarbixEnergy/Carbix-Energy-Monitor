...
const char intDirectory_P[] PROGMEM = CARBIX_INTEGRATIONS_DIR;
...
uint32_t integrator::handle_initialize_s(){
    ...
    // Insure /carbixenergymonitor/integrations exists.
    ...
    filepath += _name;
    filepath += ".log";
    _log = new CarbixLog(sizeof(intRecord), 5, 366, 32);
    trace(T_integrator,10);
    ...
    if(_intRec.UNIXtime){
        _log->readKey((CarbixLogRecord*)&_intRec);
...
uint32_t integrator::handle_integrate_s(){
    ...
    while(Current_log.lastKey() >= _intRec.UNIXtime + _interval){
        ...
        if(_newRec->UNIXtime >= _intRec.UNIXtime + _interval){
            ...
            _log->write((CarbixLogRecord *)&_intRec);
        }
...
void integrator::newLogEntry(CarbixLogRecord* oldRecord, CarbixLogRecord* newRecord){
    if(_synchronized){
...
double integrator::run(CarbixLogRecord *oldRecord, CarbixLogRecord *newRecord, units Units, char method){
    trace(T_integrator, 0);
    ...
    if(oldRecord->UNIXtime != oldInt->UNIXtime){
        ...
        int rtc = _log->readKey((CarbixLogRecord*)oldInt);
    ...
    if(newRecord->UNIXtime != newInt->UNIXtime){
        ...
        trace(T_integrator, 10);
        int rtc = _log->readKey((CarbixLogRecord*)newInt);
